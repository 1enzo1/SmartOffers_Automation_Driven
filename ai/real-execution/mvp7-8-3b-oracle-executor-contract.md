...
ACM_CUSTOM is an independent database branch. It consumes only its own runtime,
safety and operational guards; no ACM, BDA, API or smoke-summary status can
authorize or suppress it.
...
Output is one sanitized JSON object. It never includes endpoint values, host,
IP, DSN, user, password, SQL text, payload, response body or returned query
value. A successful result exposes only bounded positive validation labels,
including `fingerprint_validation=MATCH`,
`result_shape_validation=MATCH` and `sensitive_values_logged=false`. The pure
Alpha gate normalizer may map that sanitized success to
`ACM_CUSTOM_DB_CHECKPOINT_OK`; the mapped name does not release or execute any
transport.
