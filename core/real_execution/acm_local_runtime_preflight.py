"""Sanitized local integrity checks for the QA4 ACM resource."""
...
ACM_CHECKPOINT = "ORACLE_ACM_TECHNICAL_READ_ONLY_01"
ACM_ENVIRONMENT = "qa4"
ACM_PROFILE = "smartoffers_qa4_full_smoke"
ACM_RESOURCE_ID = "acm_db"
ACM_RUNTIME_READY = "ACM_RUNTIME_READY"
ACM_RUNTIME_BLOCKED = "ACM_RUNTIME_BLOCKED"
...
def preflight_acm_local_runtime(request, environ=None):
    """Return only sanitized ACM readiness metadata without external access."""
    ...
    return {
        "status": ACM_RUNTIME_READY if is_ready else ACM_RUNTIME_BLOCKED,
...
def _sql_hash_validation(environment):
    sql = _normalize_sql(_read_ref(environment, "SMARTOFFERS_QA4_ACM_SMOKE_SQL"))
    approved_hash = _read_ref(environment, "SMARTOFFERS_QA4_ACM_SMOKE_SQL_SHA256")
...
def _fingerprint_validation(environment):
    dsn = _normalize_destination(_read_ref(environment, "SMARTOFFERS_QA4_ACM_DB_DSN"))
    approved_fingerprint = _read_ref(
        environment, "SMARTOFFERS_QA4_ACM_DESTINATION_FINGERPRINT"
    )
